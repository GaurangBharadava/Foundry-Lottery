...
contract HelperConfig is CodeConstant, Script {
    ...
    function getOrCreateAnvilEthConfig() public returns (NetworkConfig memory) {
        //Check if the network already set up
        if (localNetworkConfig.vrfCoordinator != address(0)) {
            return localNetworkConfig;
        }

        //else create an anvil network
        vm.startBroadcast();
        VRFCoordinatorV2_5Mock vrfCoordianatorMock = new VRFCoordinatorV2_5Mock(
            MOCK_BASE_FEE,
            MOCK_GAS_PRICE_LINK,
        ...
        localNetworkConfig = NetworkConfig({
            entranceFee: 0.01 ether,
            interval: 30,
            vrfCoordinator: address(vrfCoordianatorMock),
            gasLane: 0x8077df514608a09f83e4e8d300645594e5d7234665448ba83f51a50f842bd3d9,
            callbackGasLimit: 500000,
            subscriptionId: 0,
